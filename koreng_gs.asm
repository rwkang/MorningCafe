; 2025.02.28 05:18 Created. 한/영 토글키 상태 표시 창 display.
section .data
...
section .text
    ...
    extern SetCapture, ReleaseCapture
...
WinMain:
    ...
    ; 클라이언트 영역 정확히 75x37으로 설정 (가로:세로 = 2:1)
    mov r14d, 75             ; 가로 75 픽셀 유지
    mov r15d, 37             ; 세로는 가로의 절반으로 설정 (75÷2=37.5, 37 픽셀로 설정)

    ; 윈도우 스타일 정의
    mov r12d, WS_POPUP | WS_VISIBLE | WS_BORDER  ; WS_THICKFRAME는 제거하여 정확한 크기 유지

    ; 윈도우 생성 - 드래그 가능하고 항상 최상위에 표시
    mov rcx, WS_EX_TOPMOST   ; dwExStyle = 항상 위에 표시
    lea rdx, [rel className]  ; lpClassName
    lea r8, [rel windowTitle] ; lpWindowName
    mov r9d, r12d             ; dwStyle - 정의된 스타일 사용
    ...
    mov dword [rsp+4], r15d   ; nHeight - 37 픽셀 (가로의 1/2)
    mov dword [rsp], r14d     ; nWidth - 75 픽셀 유지
...
.WM_NCHITTEST:
    ; 항상 캡션 영역으로 처리하여 드래그 가능하게 함
    mov rax, HTCAPTION
    jmp .end

.WM_LBUTTONDOWN:
    ; 마우스 왼쪽 버튼 누를 때 드래그 시작
    mov [rsp+32], rcx        ; hWnd 저장

    ; ReleaseCapture 호출 - 시스템이 마우스 캡처를 처리하도록 함
    call ReleaseCapture

    ; 시스템 명령 보내기 (SC_DRAGMOVE = 0xF012)
    mov rcx, [rsp+32]        ; hWnd
